export const Selectors = {
  dataRow: ".css-1liixou",
  dataCell: ".css-q34dxg",
  searchField: ".SearchFiters_containerSearch__searchBox__UwgF2",
  vehicleFilter: ".ModalOptions_blockFiltering__jpAKN",
  searchVehicleField: 'input[placeholder="Select Item(s)"]',
  vehicleFilterResult:
    ".ModalOptions_blockFiltering__trueModal__inModal__paddingModal__listItem__-6JIk",
  filterButton: ".css-1hw9j7s",

  tableRow: "table.MuiTable-root tbody.MuiTableBody-root tr",
  inputText: 'input[type="text"]',
  modalListItem:
    "div.ModalOptions_blockFiltering__trueModal__inModal__paddingModal__scrollbar__R\\+V6D li",
  tbodyRows: "tbody.MuiTableBody-root tr",
};
